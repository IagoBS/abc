...
exports.requiredIfNotExistsAll = {
    compile: helpers_1.wrapCompile(RULE_NAME, [], ([fields]) => {
        if (!fields) {
    ...
    validate(value, compiledOptions, { root, tip, errorReporter, pointer, arrayExpressionPointer }) {
        const allFieldsMissing = compiledOptions.fields.every((field) => {
            const otherFieldValue = helpers_1.getFieldValue(field, root, tip);
            return !helpers_1.exists(otherFieldValue);
        });
        if (allFieldsMissing && !helpers_1.exists(value)) {
            errorReporter.report(pointer, RULE_NAME, DEFAULT_MESSAGE, arrayExpressionPointer, { otherFields: compiledOptions.fields });
        }
    },
};
