...
exports.requiredIfNotExists = {
    compile: helpers_1.wrapCompile(RULE_NAME, [], ([field]) => {
        if (!field) {
    ...
    validate(value, compiledOptions, { root, tip, errorReporter, pointer, arrayExpressionPointer }) {
        const otherFieldValue = helpers_1.getFieldValue(compiledOptions.field, root, tip);
        const otherFieldMissing = !helpers_1.exists(otherFieldValue);
        if (otherFieldMissing && !helpers_1.exists(value)) {
            errorReporter.report(pointer, RULE_NAME, DEFAULT_MESSAGE, arrayExpressionPointer, { otherField: compiledOptions.field });
        }
    },
};
