...
class RedisDriver {
    constructor(config, redis) {
        this.config = config;
        this.redis = redis;
        this.ttl = typeof (this.config.age) === 'string' ? ms_1.default(this.config.age) : this.config.age;
    ...
    async read(sessionId) {
        ...
        const verifiedContents = new utils_1.MessageBuilder().verify(contents, sessionId);
    ...
    async write(sessionId, values) {
        ...
        await this
            .getRedisConnection()
            .setex(sessionId, this.ttl, new utils_1.MessageBuilder().build(values, undefined, sessionId));
    }
    /**
