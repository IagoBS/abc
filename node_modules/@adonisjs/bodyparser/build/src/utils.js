...
async function getFileType(fileContents, clientName, headers, force = false) {
     ...
     */
    const magicType = await file_type_1.fromBuffer(fileContents);
    if (magicType) {
     ...
     */
    return Object.assign({ ext: path_1.extname(clientName).replace(/^\./, '') }, parseMimeType(headers['content-type']));
}
